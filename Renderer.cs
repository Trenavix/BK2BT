...
class Renderer
{
    ...
    public static Vector3 GameScale = new Vector3(0.001f, 0.001f, 0.001f);
    ...
    public static void Render(Rectangle ClientRectangle, int Width, int Height, GLControl RenderPanel)
    {
        ...
        if (BinManager.ReadytoLoad && BinManager.MainBin != null)
        {
            ...
            if (BinManager.AlphaBin != null) F3DEX.ParseF3DEXDL(BinManager.AlphaBin, 1, false);
            if (EdgesOption)
            {
    ...
    public static void RenderColourBuffer(Rectangle ClientRectangle, int Width, int Height, GLControl RenderPanel)
    {
        ...
        if (BinManager.ReadytoLoad)
        {
            ...
            if (BinManager.AlphaBin != null) F3DEX.ParseF3DEXDL(BinManager.AlphaBin, 1, true);
        }
    ...
    public static void EditVertex(Rectangle ClientRectangle, int Width, int Height, GLControl RenderPanel, Point pt, byte R, byte G, byte B, byte A, bool AlphaOnly)
    {
        ...
        if (Addr != 0) for (uint i = 0; i < Vertex.CurrentVertexList.Length; i++)
            {
                bool alphabin = false;
                if (Addr > 0x80000000) alphabin = true; //remove the alpha addr flag
                uint[] list = Vertex.CurrentVertexList;
                uint rawAddr = Addr & 0x7FFFFFFF;
                BTBinFile currentbin;
                if (!alphabin) currentbin = BinManager.MainBin;
                else currentbin = BinManager.AlphaBin;
                if (Addr == Vertex.CurrentVertexList[i])
                {
                    UInt32 colour = (uint)((R << 24) | (G << 16) | (B << 8) | A);
                    if (colour == currentbin.ReadFourBytes(rawAddr + 12)) return; //If it's the same, don't do anything
...
public class Camera
{
    ...
    {
        this.MoveSpeed = 0.000004f * GameScale;
    }
